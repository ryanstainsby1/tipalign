...
Deno.serve(async (req) => {
  try {
    ...
    if (responseData.error === 'invalid_request' || responseData.error === 'invalid_grant') {
      ...
      // This is GOOD - it means credentials are valid
      return Response.json({
        success: true,
      ...
      return Response.json({
        ...
        details: [
          ...
          '  • Application was deleted or deactivated in Square Dashboard',
        ...
        recommendations: [
          ...
          '4. Generate a NEW Application Secret',
          '5. Update both in Base44 secrets'
        ]
      });
    } else {
      // Some other error
      return Response.json({
        ...
        details: [
          `Square returned: ${responseData.error || 'unknown error'}`,
          responseData.error_description || 'No description provided'
        ],
        square_response: responseData
      });
    }
